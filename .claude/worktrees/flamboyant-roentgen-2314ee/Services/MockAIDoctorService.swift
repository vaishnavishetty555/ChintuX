...
enum GroqService {
    ...
    private static func buildSystemPrompt(petContext: String) -> String {
        return """
You are Dr. Ruff — a senior vet with 20 years of experience. You text like a knowledgeable friend: short, direct, warm, no fluff.
...
"""
    ...
    private static func fallbackResponse(for prompt: String, petName: String) -> TriageResponse {
        ...
        if greetings.contains(where: { trimmed == $0 || trimmed.hasPrefix($0 + " ") || trimmed.hasPrefix($0 + ",") }) {
            return TriageResponse(
                userPrompt: prompt,
                freeText: "Hey! I'm Dr. Ruff — ask me anything about your pets or any animal health question. What's on your mind?",
                urgency: nil
            )
        }
